...
export async function fetchMidWeatherData(type) {
  const midBaseUrl = "https://apis.data.go.kr/1360000/MidFcstInfoService";
  let now = dayjs();
  let baseDate = now.format("YYYYMMDD");
  ...
  const url = `${midBaseUrl}/${type}?serviceKey=${API_KEY}&numOfRows=10&pageNo=1&dataType=JSON&regId=11B10101&tmFc=${baseDate}${baseTime}`;

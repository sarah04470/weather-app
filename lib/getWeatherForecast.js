...
export async function getCompleteForecast(lat, lon) {
  ...
  shortTermData.forEach((item) => {
    ...
    if (!dailyWeather[date]) dailyWeather[date] = { sky: "1", pty: "0", pop: "0" };
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    if (i <= 2) {
      // ✅ 1~2일차 단기 예보 사용
      const tempInfo = dailyTemp[forecastDate] || { min: "정보 없음", max: "정보 없음" };
      const weatherInfo = dailyWeather[forecastDate] || { sky: "1", pty: "0", pop: "0" };
      ...
      precipitation = weatherInfo.pop !== "0" ? `${weatherInfo.pop}%` : "강수 없음";
      humidity = shortTermData.find((item) => item.fcstDate === forecastDate && item.category === "REH")?.fcstValue ?? "정보 없음";
      windSpeed = shortTermData.find((item) => item.fcstDate === forecastDate && item.category === "WSD")?.fcstValue ?? "정보 없음";
      ...
      const morningCondition = weatherData?.[0]?.[`wf${adjustedIndex}Am`] || "맑음";
      const eveningCondition = weatherData?.[0]?.[`wf${adjustedIndex}Pm`] || "맑음";
  
      // ✅ 오전/오후 중 우선순위 높은 날씨 하나만 선택 (흐림 > 구름 많음 > 맑음)
      const priorityOrder = ["흐림", "구름 많음", "맑음"];
      weatherCondition = [morningCondition, eveningCondition]
        .sort((a, b) => priorityOrder.indexOf(a) - priorityOrder.indexOf(b))[0]; // 우선순위가 높은 값을 선택

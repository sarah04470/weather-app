...
export async function getCompleteForecast() {
  ...
  if (!shortTermData.length) return { hourly: [], tenDay: [], detailedWeather: {} };
  ...
  if (temperatureData && windSpeedData) {
    ...
    feelsLikeTemp = windSpeed > 1.3 ? (temp - windSpeed * 0.7).toFixed(1) : temp.toFixed(1);
  } else {
    feelsLikeTemp = "정보 없음";
  }

  // ✅ 강수량(PCP) 데이터 가져오기
  const precipitationData = shortTermData.find(item => item.category === "PCP");
  let precipitation;
  if (precipitationData) {
    precipitation = precipitationData.fcstValue === "강수없음" ? "0" : precipitationData.fcstValue;
  } else {
    precipitation = "정보 없음";
  }
  ...
  const detailedWeather = {
    ...
    precipitationProb: shortTermData.find((item) => item.category === "POP")?.fcstValue ?? "N/A",
    humidity: shortTermData.find((item) => item.category === "REH")?.fcstValue ?? "N/A",
    windSpeed: shortTermData.find((item) => item.category === "WSD")?.fcstValue ?? "N/A",
    windDirection: getWindDirection(shortTermData.find((item) => item.category === "VEC")?.fcstValue),
  };

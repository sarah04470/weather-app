...
export async function getWeatherData(date, time, nx, ny) {
  ...
  try {
    ...
    const weather = {
      temperature:
        data.find((item) => item.category === "TMP")?.fcstValue || "N/A",
      humidity:
        data.find((item) => item.category === "REH")?.fcstValue || "N/A",
      windSpeed:
        data.find((item) => item.category === "WSD")?.fcstValue || "N/A",
      description:
        data.find((item) => item.category === "SKY")?.fcstValue === "1"
          ? "맑음"
          : "흐림",
    };
